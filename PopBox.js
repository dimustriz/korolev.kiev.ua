/********************************************************************************************************************
* PopBox.js, v2.6 Copyright (c) 2008, C6 Software, Inc. (http://www.c6software.com/)
...
var popBoxRevertText = "Щёлкните для уменьшения картинки";
...
var popBoxCaptionMoreText = " Еще...";
var popBoxCaptionLessText = "Прибрать...";
...
function PopInPlace(obj, speed, className)
{
	...
	// figure out the maximum width and height of the new image
	var scrollOffset = GetScrollOffset();
	var leftMargin = startX - scrollOffset.x;
	var rightMargin = windowSize.x - (leftMargin + popBox[elem.id].wOriginal);
	var topMargin = startY - scrollOffset.y;
	var bottomMargin = windowSize.y - (topMargin + popBox[elem.id].hOriginal);
	var maxWSize = (Math.min(leftMargin, rightMargin) * 2) + popBox[elem.id].wOriginal;
	var maxHSize = (Math.min(topMargin, bottomMargin) * 2) + popBox[elem.id].hOriginal;
	...
	// some browsers have a race condition where it still doesn't get set so set it to the maximum
	if (newWidth == 0 || newHeight == 0)
	{
		var scale = Math.min(parseFloat(maxWSize) / parseFloat(elem.width), parseFloat(maxHSize) / parseFloat(elem.height));
		newWidth = parseInt(elem.width * scale);
		newHeight = parseInt(elem.height * scale);
	}

	// scale image to fit if necessary
	if (newWidth > maxWSize || newHeight > maxHSize)
	{
		var scale = Math.min(parseFloat(maxWSize) / parseFloat(newWidth), parseFloat(maxHSize) / parseFloat(newHeight));
		newWidth = parseInt(newWidth * scale);
		newHeight = parseInt(newHeight * scale);
	}
	
	// get new position
	var newLeft = startX - ((newWidth - popBox[elem.id].wOriginal) / 2);
	var newTop = startY - ((newHeight - popBox[elem.id].hOriginal) / 2);
